...
plot.clustering <- function(inputfile, bin.bed.filename, position.outputfile, chromosome.outputfile) {
    ...
    #Specify library and removed cex to remove collision
    res <- pheatmap::pheatmap(result, border_color = NA, show_rownames = T, show_colnames = F, cluster_cols = F, cluster_rows = T, clustering_method = "ward.D", scale = "none", col = mycol, main = inputfile, annotation_col = col_annotation, annotation_colors = anno_colors, breaks = breaksList, annotation_legend = FALSE)
    ...
    #print(result_sv)
    mat <- as.matrix(result_sv[res$tree_row$order, ])
    ...
    #Copy Pasted from previous plot-clustering.R
	data1<-read.table(inputfile, sep = '\t', header=T, comment.char = "")
	...
	res<-pheatmap::pheatmap(result, border_color = NA, show_rownames=T, show_colnames=F, cluster_cols=F, cluster_rows=T, clustering_method="ward.D", scale="none", col=mycol, cex=0.5, main=inputfile, breaks = breaksList)
